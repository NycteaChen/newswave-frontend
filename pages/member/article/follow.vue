<template>
  <div class="member-follow mx-auto mx-md-0">
    <p class="text-muted fs-sm">追蹤感興趣的新聞主題，NewsWave 將通知您最新相關主題文章，讓您掌握第一時間資訊！</p>
...
function toggleFollowBtn(name: string) {
  if (follows.value?.includes(name)) {
    deleteFollowNewsTopicHandler(name);
  } else {
    followNewsTopicHandler(name);
  }
}
</script>
